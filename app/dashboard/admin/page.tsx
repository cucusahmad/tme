...
export default async function AdminDashboardPage() {
  ...
  const cards = [
    { label: "Total Pengguna", value: totalUsers, note: `${activeUsers} akun aktif`, icon: Users, tone: "from-emerald-500 to-emerald-700", soft: "bg-emerald-50 text-emerald-600" },
    { label: "Profil Lengkap", value: completedProfiles, note: `${profileRate}% dari pengguna`, icon: UserCheck, tone: "from-emerald-500 to-emerald-700", soft: "bg-emerald-50 text-emerald-700" },
  ...
  return (
    <div className="space-y-7">
      ...
      <section className="relative overflow-hidden rounded-[2rem] bg-emerald-950 px-6 py-8 text-white shadow-xl shadow-slate-200 sm:px-9">
        <div className="absolute -right-20 -top-24 h-72 w-72 rounded-full bg-emerald-500/20 blur-3xl" />
        <div className="absolute bottom-0 right-1/3 h-36 w-36 rounded-full bg-emerald-500/20 blur-3xl" />
        <div className="relative flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
          <div className="max-w-2xl">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1.5 text-xs font-semibold text-emerald-200">
              <Sparkles size={14} /> Future Education & Career ? Administrator
            </div>
            <h1 className="text-3xl font-black tracking-tight sm:text-4xl">Selamat datang, Administrator</h1>
            <p className="mt-3 max-w-xl text-sm leading-6 text-slate-300 sm:text-base">Dampingi perjalanan peserta dari pendidikan dan pengembangan kompetensi hingga kesiapan karier.</p>
          </div>
          <Link href="/dashboard/admin/users" className="inline-flex w-fit items-center gap-2 rounded-xl bg-amber-300 px-5 py-3 text-sm font-bold text-slate-950 transition hover:bg-amber-200">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.45fr_0.75fr]">
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-5 sm:px-6">
            ...
            <Link href="/dashboard/admin/users" className="text-sm font-semibold text-emerald-700 hover:text-emerald-800">Lihat semua</Link>
          </div>
          <div className="divide-y divide-slate-100">
            {recentUsers.map((user) => {
              ...
              return <div key={user.user_id.toString()} className="flex items-center gap-3 px-5 py-4 sm:px-6">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-emerald-100 to-emerald-100 font-bold text-emerald-700">{(user.biodata?.nama_lengkap || user.email).charAt(0).toUpperCase()}</div>
                ...
                <div className="hidden text-right sm:block"><span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold ${completed ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{completed ? <CheckCircle2 size={12}/> : <Clock3 size={12}/>} {completed ? "Selesai" : "Belum selesai"}</span><p className="mt-1 text-xs text-slate-400">{user.created_at ? dateFormatter.format(user.created_at) : "-"}</p></div>
              </div>;
            })}
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-7 space-y-6">
            {[{ label: "Kelengkapan profil", value: profileRate, color: "bg-emerald-600" }, { label: "Assessment selesai", value: completionRate, color: "bg-emerald-500" }, { label: "Rekomendasi tersedia", value: totalUsers ? Math.round((recommendations / totalUsers) * 100) : 0, color: "bg-emerald-500" }].map((item) => (
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col justify-between gap-3 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-center sm:px-6">
          ...
          <Link href="/dashboard/admin/users" className="text-sm font-semibold text-emerald-700 hover:text-emerald-800">Lihat seluruh pengguna</Link>
        </div>
