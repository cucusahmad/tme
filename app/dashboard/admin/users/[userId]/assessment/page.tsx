...
export default async function UserAssessmentPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 p-7 text-white shadow-xl sm:p-9"><div className="absolute -right-16 -top-16 h-52 w-52 rounded-full bg-emerald-400/20 blur-3xl"/><div className="relative"><span className="inline-flex items-center gap-2 rounded-full bg-emerald-400/15 px-3 py-1.5 text-xs font-bold text-emerald-300"><CheckCircle2 size={14}/>Assessment selesai</span><h1 className="mt-5 text-3xl font-black">{user.biodata?.nama_lengkap || "Pengguna"}</h1><p className="mt-1 text-sm text-slate-300">{user.email} · {user.biodata?.profession?.profession_name || "Profesi belum dipilih"}</p><p className="mt-5 text-xs text-slate-400">Diselesaikan {assessment.completed_at ? date.format(assessment.completed_at) : "-"}</p></div></section>
