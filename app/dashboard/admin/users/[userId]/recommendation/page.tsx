...
export default async function UserRecommendationPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  return <div className="space-y-6">
    <Link href="/dashboard/admin/users" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-emerald-700"><ArrowLeft size={17}/>Kembali ke data pengguna</Link>
    <section className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-emerald-950 via-slate-950 to-emerald-950 p-7 text-white shadow-xl sm:p-9"><div className="absolute -right-12 -top-20 h-64 w-64 rounded-full bg-violet-500/20 blur-3xl"/><div className="relative"><span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1.5 text-xs font-bold text-emerald-200"><BrainCircuit size={14}/>Rekomendasi berbasis AI</span><h1 className="mt-5 text-3xl font-black">Rencana Pengembangan Individu</h1><p className="mt-2 text-slate-300">{user.biodata?.nama_lengkap || user.email} · {user.biodata?.profession?.profession_name || "Talent Match Ecosystem"}</p></div></section>
    ...
    {development && <section className="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-6"><h2 className="flex items-center gap-2 font-bold text-emerald-950"><Compass size={20}/>Arahan Pengembangan</h2><p className="mt-3 text-sm leading-7 text-slate-700">{text(development.summary)}</p>{Array.isArray(development.recommendations)&&<ul className="mt-4 grid gap-2 md:grid-cols-2">{development.recommendations.map((item,index)=><li key={index} className="flex gap-2 rounded-lg bg-white p-3 text-sm text-slate-700"><CheckCircle2 size={17} className="mt-0.5 shrink-0 text-emerald-600"/>{text(item)}</li>)}</ul>}</section>}
    ...
    {learning.length>0&&<section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="mb-5 flex items-center gap-2 font-bold text-slate-900"><BookOpen size={20} className="text-emerald-600"/>Learning Path</h2><div className="grid gap-3 md:grid-cols-2">{learning.map((item,index)=><div key={index} className="rounded-xl border border-emerald-100 bg-emerald-50/50 p-4"><h3 className="font-bold text-slate-900">{text(item.title)}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{text(item.description)}</p></div>)}</div></section>}
    {commitment&&<section className="rounded-2xl bg-gradient-to-r from-slate-900 to-emerald-950 p-7 text-white"><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-300">Komitmen personal</p><h2 className="mt-3 text-xl font-black">{text(commitment.title)}</h2><p className="mt-3 max-w-3xl text-sm italic leading-7 text-slate-300">“{text(commitment.statement)}”</p></section>}
  </div>;
}
