...
export default function PublicAssessment({ token }: { token: string }) {
  ...
  if (!sessionToken) return (
    <div className="mx-auto max-w-xl rounded-3xl border border-slate-200 bg-white p-7 shadow-lg sm:p-9">
      <span className="inline-flex items-center gap-2 rounded-full bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700"><ShieldCheck size={15} /> Tanpa login</span>
      ...
      {owner.profession && <p className="mt-3 text-sm font-semibold text-blue-700">Profesi target: {owner.profession}</p>}
      <form onSubmit={start} className="mt-7 space-y-4">
        <label className="block text-sm font-semibold text-slate-700">Nama Anda<input value={name} onChange={(event) => setName(event.target.value)} required minLength={2} maxLength={120} className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" placeholder="Contoh: Budi Santoso" /></label>
        <label className="block text-sm font-semibold text-slate-700">Hubungan dengan {owner.ownerName}<input value={relationship} onChange={(event) => setRelationship(event.target.value)} required minLength={2} maxLength={80} className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" placeholder="Contoh: Rekan kerja, atasan, teman" /></label>
        {error && <p className="text-sm font-medium text-red-600">{error}</p>}
        <button disabled={submitting} className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-3 font-semibold text-white transition hover:bg-blue-700 disabled:bg-blue-400">{submitting && <LoaderCircle className="animate-spin" size={18} />} Mulai penilaian</button>
      </form>
    </div>
  );
