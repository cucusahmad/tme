...
import { BarChart3, BookOpenCheck, BriefcaseBusiness, ClipboardCheck, LayoutDashboard, ShieldCheck, UserRound, X } from "lucide-react";
...
export default function MobileSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <><div onClick={onClose} className={`fixed inset-0 z-40 bg-slate-950/60 backdrop-blur-sm transition lg:hidden ${open ? "visible opacity-100" : "invisible opacity-0"}`} /><aside className={`fixed left-0 top-0 z-50 flex h-dvh w-[min(88vw,20rem)] flex-col bg-[#07182d] text-white shadow-2xl transition-transform duration-300 lg:hidden ${open ? "translate-x-0" : "-translate-x-full"}`}>
    <div className="flex h-24 items-center justify-between border-b border-white/10 px-5"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-amber-400 text-[#07182d]"><ShieldCheck size={23} /></div><div><p className="text-[9px] font-bold uppercase tracking-[.24em] text-amber-300">Talent Match</p><h2 className="font-black">SIPETA POLRI</h2></div></div><button onClick={onClose} aria-label="Tutup navigasi" className="rounded-xl border border-white/10 p-2 text-slate-400"><X size={19} /></button></div>
