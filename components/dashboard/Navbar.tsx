...
export default function Navbar({ onOpenSidebar }: { onOpenSidebar?: () => void }) {
  ...
  return <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-[#f3f6f9]"><div className="flex h-20 items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
    ...
    <div className="flex shrink-0 items-center gap-2 sm:gap-3"><button aria-label="Notifikasi" className="relative flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:border-amber-300 hover:text-amber-700"><Bell size={18} /><span className="absolute right-2.5 top-2 h-2 w-2 rounded-full bg-amber-500 ring-2 ring-white" /></button><div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-1.5 pr-3 shadow-sm"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#0b2744] text-amber-300"><UserRound size={18} /></div><div className="hidden sm:block"><p className="text-xs font-bold text-slate-800">Pengguna Talent Match</p><p className="flex items-center gap-1 text-[10px] text-slate-500"><ShieldCheck size={11} className="text-emerald-600" />Akun terverifikasi</p></div></div></div>
  </div></header>;
}
