...
export default function CaraKerja() {
  return <section id="cara-kerja" className="bg-[#faf7ee] py-24 sm:py-32"><div className="mx-auto max-w-7xl px-6 lg:px-8">
    ...
    <div className="relative mt-16 grid gap-5 md:grid-cols-2 lg:grid-cols-4"><div className="absolute left-[12%] right-[12%] top-10 hidden h-px bg-slate-300 lg:block" />{steps.map(({icon:Icon,title,text},i)=><article key={title} className="relative rounded-2xl border border-slate-200 bg-white p-7"><div className="relative z-10 flex h-20 w-20 items-center justify-center rounded-2xl border-8 border-[#faf7ee] bg-[#115e50] text-amber-300"><Icon size={25}/></div><p className="mt-6 text-xs font-black uppercase tracking-[.18em] text-amber-600">Tahap 0{i+1}</p><h3 className="mt-3 text-xl font-bold text-[#115e50]">{title}</h3><p className="mt-3 text-sm leading-7 text-slate-600">{text}</p></article>)}</div>
