...
const dimensions = [{ label: "Kepemimpinan", value: 88 }, { label: "Analisis", value: 94 }, { label: "Manajerial", value: 81 }, { label: "Komunikasi", value: 85 }];
    ...
    <section id="beranda" className="relative min-h-screen overflow-hidden pt-32 lg:pt-36">
      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100vh-9rem)] max-w-7xl items-center gap-14 px-6 pb-20 lg:grid-cols-[1.08fr_.92fr] lg:px-8">
        ...
        <div className="relative mx-auto w-full max-w-xl">
          <div className="relative overflow-hidden rounded-[2rem] border border-white/15 bg-[#142c42] p-5 shadow-2xl sm:p-7">
            <div className="mb-7 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-amber-300">Profil Talenta Anda</p><p className="mt-2 text-lg font-bold text-white">Ringkasan profil kompetensi</p></div><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-amber-300 text-[#071426]"><Radar size={23} /></div></div>
            <div className="grid gap-3 sm:grid-cols-3">
              {[{ icon: UserRoundCheck, value: "Profil", label: "Data individu" }, { icon: Activity, value: "Asesmen", label: "Potensi & kompetensi" }, { icon: BriefcaseBusiness, value: "Arah", label: "Rekomendasi peran" }].map(({icon: Icon, value, label}) => <div key={value} className="rounded-2xl border border-white/10 bg-[#061221]/60 p-4"><Icon size={19} className="mb-4 text-amber-300" /><p className="font-bold text-white">{value}</p><p className="mt-1 text-xs text-slate-400">{label}</p></div>)}
            </div>
            ...
            <div className="mt-4 rounded-2xl border border-cyan-300/20 bg-cyan-300/10 p-5"><p className="text-xs font-semibold text-cyan-200">Contoh kecocokan profesi</p><div className="mt-2 flex items-center justify-between gap-3"><p className="font-bold text-white">Analis Bisnis</p><span className="text-lg font-black text-amber-300">89%</span></div><p className="mt-2 text-xs leading-5 text-slate-300">Kekuatan analisis dan komunikasi mendukung eksplorasi peran ini.</p></div>
            <p className="mt-4 text-center text-[11px] text-slate-500">Contoh ilustrasi, bukan hasil asesmen pengguna</p>
          </div>
        </div>
      </div>
    </section>
