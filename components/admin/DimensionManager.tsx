...
export default function DimensionManager({ dimensions, professions }: { dimensions: Dimension[]; professions: Profession[] }) {
  ...
  return <div className="space-y-6">
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-5 flex items-center gap-3"><div className="rounded-xl bg-cyan-50 p-2.5 text-cyan-700"><Plus size={20}/></div><div><h2 className="font-bold text-slate-900">Tambah Dimensi</h2><p className="text-xs text-slate-500">Tentukan profesi, nama, dan urutan tampil.</p></div></div>
      <form action={createAction} className="grid gap-4 md:grid-cols-[1fr_1.4fr_140px_auto] md:items-end">
        <label className="space-y-1.5 text-sm font-semibold text-slate-700">Profesi<select name="professionId" required defaultValue="" className="block w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 font-normal outline-none focus:border-cyan-500"><option value="" disabled>Pilih profesi</option>{professions.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        <label className="space-y-1.5 text-sm font-semibold text-slate-700">Nama dimensi<input name="dimensionName" required maxLength={100} placeholder="Contoh: Kepemimpinan" className="block w-full rounded-xl border border-slate-200 px-3 py-2.5 font-normal outline-none focus:border-cyan-500"/></label>
        <label className="space-y-1.5 text-sm font-semibold text-slate-700">Urutan<input name="orderNo" type="number" min={0} placeholder="Opsional" className="block w-full rounded-xl border border-slate-200 px-3 py-2.5 font-normal outline-none focus:border-cyan-500"/></label>
