...
export default function AdminShell({
  ...
  const sidebar = (
    <div className="flex h-full flex-col bg-emerald-950 text-white">
      ...
      <p className="px-6 pb-2 pt-6 text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Menu Utama</p>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 rounded-xl px-4 py-3 font-medium transition ${
                active ? "bg-gradient-to-r from-emerald-600 to-teal-700 text-white shadow-lg shadow-emerald-950/50" : "text-slate-300 hover:bg-white/10 hover:text-white"
              }`}
            >
              <Icon size={20} /> {label}
            </Link>
      ...
      <div className="lg:ml-72">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-slate-200/80 bg-white px-5 lg:px-8">
          ...
          <div className="ml-auto flex items-center gap-4">
            <button aria-label="Notifikasi" className="relative rounded-xl border border-slate-200 bg-white p-2.5 text-slate-500"><Bell size={18}/><span className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-emerald-500"/></button>
